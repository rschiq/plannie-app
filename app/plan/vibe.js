    ...
    if (plan.mode === 'auto') {
      ...
      try {
        ...
        const addonItem = addonRaw ? {
          ...
          shortLocation: shortenVicinity(addonRaw.vicinity || ''),
          desc:         shortenVicinity(addonRaw.vicinity || '') || 'Near your location',
          address:      addonRaw.vicinity || '',
          ...
          location:     addonRaw.geometry?.location
            ? { lat: addonRaw.geometry.location.lat, lng: addonRaw.geometry.location.lng } : null,
          photoUrl:     addonRaw.photos?.[0]?.photo_reference
            ? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${addonRaw.photos[0].photo_reference}&key=AIzaSyBuaZy0PskAbddfeyxarwdMRsUa6WiRP9w`
            : null,
        } : null;
...
const loader = StyleSheet.create({
  ...
  box: { width: 280, borderRadius: 28, padding: 40, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(212,149,111,0.15)' },
  glowRing: { width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(212,149,111,0.10)', alignItems: 'center', justifyContent: 'center', marginBottom: 20, borderWidth: 1, borderColor: 'rgba(212,149,111,0.20)' },
  emoji:   { fontSize: 36 },
