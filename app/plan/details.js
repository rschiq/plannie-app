...
export default function DetailsScreen() {
  ...
  function handleNext() {
    updatePlan({
      date: dateVal, dateDisplay: formatDateDisplay(dateVal),
      city: cityVal, time: timeVal, timeDisplay: fmtTime(timeVal),
    });
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScreenHeader title={"Let's build\n"} italic="your date."
        subtitle="When and where are you headed?"
        onBack={() => router.push('/(tabs)/')} />
      <ProgressBar total={7} current={1} />
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.fg}>
          <Text style={styles.fl}>Date</Text>
          <TextInput style={styles.fi} value={dateVal} onChangeText={setDateVal}
            placeholder="YYYY-MM-DD" placeholderTextColor={colors.gray3} />
        </View>
              ...
              <View style={styles.dropdown}>
                {suggestions.map((c, i) => (
                  <TouchableOpacity key={i}
                    ...
                    onPress={() => pickCity(c)} activeOpacity={0.8}>
        ...
        <View style={styles.fg}>
          ...
          <TextInput style={styles.fi} value={timeVal} onChangeText={setTimeVal}
            placeholder="HH:MM  (e.g. 17:00 = 5 PM)"
            placeholderTextColor={colors.gray3} keyboardType="numbers-and-punctuation" />
...
const styles = StyleSheet.create({
  ...
  dropdown: { position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: colors.white, borderRadius: radius.sm, borderWidth: 1, borderColor: colors.cream2, zIndex: 99, marginTop: 4, ...shadow.lg },
